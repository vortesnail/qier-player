...
const danmaku = ref<Danmaku | null>(null);
...
const initPlayer = () => {
  ...
  player.value?.on(EVENT.TIME_UPDATE, () => {
...
const initDanmaku = () => {
  ...
  danmaku.value = new Danmaku(danmuWrapper, {
    // @ts-ignore
    eventProxyElement: danmuWrapper,
    duration: 15000
  })

  player.value.on(EVENT.PLAY, () => {
    danmaku.value?.start();
  });

  player.value.on(EVENT.PAUSE, () => {
    danmaku.value?.stop();
  });
}
...
watch(
  () => lang.value,
  () => {
    I18n.setCurrentLang(lang.value);
    if (player.value) {
